...
if __name__ == '__main__':
    rospy.init_node('energy_hierarchy_hybrid', anonymous=True)
    logfile = open('/home/czx/hybrid_uavs.txt', 'w')
    if not use_anchor:
        uav_handles = []
        phi_list = [0.0] * 12
        r_list = [0.0] * 12
        targets = [[0., 0.]] * 12
        for i in range(0, 12):
            if i in [4,5,6,7,10,11]:
                uav_handles.append(UAVhandleFixedWing(i))
            else:
        ...
        for i in range(0,5000):
            ...
            plane_v = lpvf.phi_sync_control(phi_list=phi_list, v0=8.0, Kdv=2.0)
            for j in range(0, 12):
                ...
                vx_d, vy_d, phi_d, diff = lpvf.lpvf(8.0, uav_handles[j].px, uav_handles[j].py, targets[j][0], targets[j][1],
                                              32.0)
                if j in [0,1,2,3,8,9]:
                    psi_t = phi_d
                    # psi_t = psi+w_t*step
                    ...
                    if psi_t < -np.pi:
                        psi_t += 2 * np.pi
                    u2 = -lpvf.standAngleDiff(uav_handles[j].yaw, psi_t) + diff
                    msg = Twist()
                    #msg.linear.x = plane_v[j]
                    #msg.angular.z = u2
                    if plane_v[j]>11.0:
                        plane_v[j] = 11.0

                    msg.linear.x = vx_d / 8.0 * plane_v[j]
                    msg.linear.y = vy_d / 8.0 * plane_v[j]
                    ...
                    vx_d, vy_d, phi_d, diff = lpvf.lpvf(8.0, uav_handles[j].px, uav_handles[j].py, targets[j][0],
                                                        targets[j][1],
                                                        30)
                    psi_t = -phi_d
                    if plane_v[j]>15:
            ...
            s_error_t, s_error_a = oe.squire_error(40, uav_handles[8].px, uav_handles[8].py, uav_handles[9].px,
                                                   uav_handles[9].py, uav_handles[10].px, uav_handles[10].py,
                                                   uav_handles[11].px, uav_handles[11].py)
            t1_error_t, t1_error_a = oe.triangle_error(20, uav_handles[8].px, uav_handles[8].py, uav_handles[0].px,
                                                       uav_handles[0].py, uav_handles[1].px, uav_handles[1].py)
            t2_error_t, t2_error_a = oe.triangle_error(20, uav_handles[9].px, uav_handles[9].py, uav_handles[2].px,
                                                       uav_handles[2].py, uav_handles[3].px, uav_handles[3].py)
            t3_error_t, t3_error_a = oe.triangle_error(20, uav_handles[10].px, uav_handles[10].py, uav_handles[4].px,
                                                       uav_handles[4].py, uav_handles[5].px,
                                                       uav_handles[5].py)
            t4_error_t, t4_error_a = oe.triangle_error(20, uav_handles[11].px, uav_handles[11].py, uav_handles[6].px,
                                                       uav_handles[6].py, uav_handles[7].px,
                                                       uav_handles[7].py)

            t1_e_t, t1_e_a = oe.triangle_error(20, uav_handles[8].px, uav_handles[8].py, uav_handles[0].px,
                                               uav_handles[0].py, uav_handles[1].px, uav_handles[1].py)
            t2_e_t, t2_e_a = oe.triangle_error(20, uav_handles[8].px + 40, uav_handles[8].py, uav_handles[2].px,
                                               uav_handles[2].py, uav_handles[3].px, uav_handles[3].py)
            t3_e_t, t3_e_a = oe.triangle_error(20, uav_handles[8].px, uav_handles[8].py + 40, uav_handles[4].px,
                                               uav_handles[4].py, uav_handles[5].px, uav_handles[5].py)
            t4_e_t, t4_e_a = oe.triangle_error(20, uav_handles[8].px + 40, uav_handles[8].py + 40, uav_handles[6].px,
                                               uav_handles[6].py, uav_handles[7].px, uav_handles[7].py)
            ...
            logfile.write(str(i) + ' ' + str(rospy.get_time()) + ' ' + str(all_e_a) + ' ' + str(s_error_a) + ' ' + str(
                alltri_e_a) + ' ' + str(
                intertri_e_a) + '\n')
            rospy.sleep(0.02)
        exit(1)